    ...
    def create_mapper(self, projections, lens=None):
        ...
        try:
            if lens_dim == 3:
                ...
                cover = km.Cover(
                    n_cubes=self.n_cubes * 3, perc_overlap=self.perc_overlap
                )
            else:
        ...
        for clusterer_name, clusterer in clusterers_to_try:
            try:
                ...
                complex_candidate = self.mapper.map(
                    lens_normalized, X=projections, cover=cover, clusterer=clusterer
                )
                ...
                if n_nodes > 0:
                    ...
                    temp_metrics = self.compute_lens_metrics(
                        lens_normalized, projections, complex_candidate
                    )
                    ...
                    h1_score = (
                        min(h1_features, 5) * 2
                    )  # Reward 1-5 loops, diminishing returns after
                    ...
                    h0_score = max(
                        0, 10 - abs(h0_features - 5)
                    )  # Optimal around 5 components
                    ...
                    print(
                        f"        H0: {h0_features}, H1: {h1_features}, persist: {persistence_mean:.3f}, score: {score:.2f}"
                    )
        ...
        if best_complex is None:
            ...
            best_complex = self.mapper.map(
                ...
                cover=cover,
                clusterer=KMeans(
                    n_clusters=2, random_state=self.random_state, n_init=10
                ),
            )
        ...
        try:
            ...
            if not is_connected:
                print(
                    f"    ⚠️ Graph is disconnected! This may cause visualization differences."
                )
        except Exception as e:
            print(f"    Could not analyze connectivity: {e}")

        # Compute and store metrics
        metrics = self.compute_lens_metrics(
            lens_normalized, projections, simplicial_complex
        )
        ...
        simplicial_complex["meta"][
            "lens_data"
        ] = lens_normalized  # Store lens data for H1 computation
            ...
            try:
                ...
                if np.sum(non_noise_mask) > 1:
                    score = silhouette_score(
                        lens[non_noise_mask], cluster_labels[non_noise_mask]
                    )
        ...
        try:
            ...
            if len(lens) > max_points_for_barcode:
                indices = np.random.choice(
                    len(lens), max_points_for_barcode, replace=False
                )
            ...
            h1_persistence = (
                diagrams[1] if len(diagrams) > 1 else np.array([]).reshape(0, 2)
            )
            ...
            metrics["h0_max_lifetime"] = (
                np.max(h0_lifetimes) if len(h0_lifetimes) > 0 else 0.0
            )
            metrics["h0_mean_lifetime"] = (
                np.mean(h0_lifetimes) if len(h0_lifetimes) > 0 else 0.0
            )

            metrics["h1_num_features"] = len(h1_persistence)
            metrics["h1_max_lifetime"] = (
                np.max(h1_lifetimes) if len(h1_lifetimes) > 0 else 0.0
            )
            metrics["h1_mean_lifetime"] = (
                np.mean(h1_lifetimes) if len(h1_lifetimes) > 0 else 0.0
            )

            # Overall persistence score (combination of H0 and H1)
            all_lifetimes = (
                np.concatenate([h0_lifetimes, h1_lifetimes])
                if len(h1_lifetimes) > 0
                else h0_lifetimes
            )
            metrics["persistence_mean"] = (
                np.mean(all_lifetimes) if len(all_lifetimes) > 0 else 0.0
            )
            metrics["persistence_std"] = (
                np.std(all_lifetimes) if len(all_lifetimes) > 0 else 0.0
            )
...
class PlotEngine:
    def __init__(self, output_dir="mapper_results"):
        ...
        self.mapper = km.KeplerMapper(
            verbose=0
        )  # Add mapper instance for visualization

    def create_static_visualization(
        self, simplicial_complex, optimizer, metrics, lens_data=None
    ):
        print(f"🎨 Creating static visualization for {optimizer}...")

        # Compute H1 persistence for each node as color function
        node_colors = self._compute_node_h1_persistence(
            simplicial_complex, metrics, lens_data
        )
        ...
        print(
            "    Note: Node sizes show cluster size, color shows H1 persistence conceptually"
        )
        ...
        if len(node_colors) > 0:
            ...
            cbar = plt.colorbar(
                ...
                ax=ax,
                label="H1 Persistence",
            )
            print(
                f"    H1 persistence range: {min(color_values):.3f} to {max(color_values):.3f}"
            )
    ...
    def create_interactive_visualization(
        self, simplicial_complex, optimizer, metadata, lens_data=None
    ):
        ...
        try:
            ...
            total_points = sum(
                len(members) for members in simplicial_complex["nodes"].values()
            )
            ...
            print(
                f"    Color function range: {np.min(color_function):.3f} to {np.max(color_function):.3f}"
            )
            ...
            try:
                ...
                print(
                    f"    ⚠️ Color function not supported, using default: {color_error}"
                )
    ...
    def _compute_node_h1_persistence(self, simplicial_complex, metrics, lens_data=None):
        ...
        if lens_data is None:
            ...
            max_cluster_size = (
                max(len(members) for members in nodes.values()) if nodes else 1
            )
        ...
        try:
            for node_id, members in nodes.items():
                ...
                if len(node_data) > max_points_per_node:
                    indices = np.random.choice(
                        len(node_data), max_points_per_node, replace=False
                    )
            ...
            print(
                f"    Node colors (actual H1 persistence): min={min(node_colors.values()):.3f}, max={max(node_colors.values()):.3f}"
            )
...
class MapperAnalyzer:
    ...
    def analyze_optimizer(self, optimizer, trajectories, metadata):
        ...
        simplicial_complex = self.mapper_engine.create_mapper(
            projections["pca_50"], lens=lens
        )
...
class AnalysisEngine:
    def __init__(
        self, max_trajectories=620, max_total=30000, output_dir="mapper_results"
    ):
    ...
    def run_analysis(self, optimizers, create_visualizations=True):
        ...
        for optimizer in optimizers:
            ...
            # Headless analysis
            result = self.analyzer.analyze_optimizer(
                optimizer, trajectories, valid_metadata
            )
            ...
            if create_visualizations and result is not None:
                ...
                self.plot_engine.create_static_visualization(
                    simplicial_complex, optimizer, metrics, lens_data
                )
                self.plot_engine.create_interactive_visualization(
                    simplicial_complex, optimizer, valid_metadata, lens_data
                )
        ...
        print(
            f"📊 Analyzed {len([r for r in all_results.values() if r is not None])} optimizers"
        )
