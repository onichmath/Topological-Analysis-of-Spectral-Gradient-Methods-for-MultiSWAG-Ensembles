...
class GradientTracker:
    def __init__(
        self, save_dir: str, optimizer_name: str, max_batches_tracked: int = 50
    ):
        ...
        self.max_batches_tracked = max_batches_tracked
        self.epoch_norms = {}
        ...
        for particle_key in self.epoch_norms.keys():
            ...
            if batch_norms_list:
                ...
                for norm_name in norm_names:
                    norm_values = [
                        batch_norms[norm_name]
                        for batch_norms in batch_norms_list
                        if norm_name in batch_norms
                    ]
    ...
    def save_epoch_statistics(self, epoch_stats: Dict):
        ...
        norms_filepath = os.path.join(
            self.gradients_dir, f"epoch{self.current_epoch}_norm_stats.pt"
        )
        torch.save(summary_stats, norms_filepath)

        print(
            f"Saved gradient norm statistics for epoch {self.current_epoch} ({self.batch_count} batches tracked)"
        )
...
class LossLandscapeTracker:
    ...
    def save_loss_components(
        ...
        filepath = os.path.join(
            self.landscape_dir, f"particle{particle_id}_epoch{epoch}_losses.pt"
        )
...
def setup_comprehensive_tracking(optimizer_name: str):
    ...
    return {
        "gradient_tracker": GradientTracker(
            base_dir, optimizer_name, max_batches_tracked=50
        ),
