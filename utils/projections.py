...
class DataLoader:
    ...
    def load_data(self, optimizers, epochs=range(0, 31), max_particles=20):
        ...
        for optimizer in optimizers:
            ...
            if len(optimizer_files) > self.max_trajectories:
                print(f"Subsampling {optimizer_files} to {self.max_trajectories} trajectories")
    ...
    def load_optimizer_trajectories(self, optimizer, epochs=range(0, 31), max_particles=20):
...
class ProjectionEngine:
    ...
    @staticmethod
    def save_projections(projections, lens_functions, metadata, optimizer, results_dir="results"):
...
def generate_projections_for_optimizer(optimizer, data_loader, projection_engine, include_tsne=True):
    """Generate and save projections for a single optimizer."""
    print(f"\nGENERATING PROJECTIONS FOR {optimizer.upper()}")
    print("-" * 50)
    
    # Load trajectories for this optimizer
    ...
    filename = projection_engine.save_projections(projections, lens_functions, metadata, optimizer)
...
def generate_optimizer_projections():
    ...
    print(f"\nSUMMARY TABLE:")
    print("-" * 80)
    print(f"{'Optimizer':<15} {'Trajectories':<12} {'Projections':<15} {'Lens Functions':<15}")
    print("-" * 80)
    
    for optimizer, result in all_results.items():
        projections_str = f"{len(result['projections'])}"
        lens_str = f"{len(result['lens_functions'])}"
        print(f"{optimizer:<15} {result['n_trajectories']:<12} {projections_str:<15} {lens_str:<15}")
